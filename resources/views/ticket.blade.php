        ...
        @foreach ($tickets as $ticket)
            <div class="card rounded-1">
                <div class="card-body d-flex flex-column">
                    ...
                    <p class="card-text fw-bold">No. Kursi : 34</p>
                    <form class="align-self-end" action="{{ route('destroy', $ticket)  }}" method="POST">
                        @method('delete')
                        @csrf
                        <button class="btn btn-danger fw-bold p-1">Batalkan Tiket</button>
                    </form> 
                </div>
            </div>
        @endforeach    
